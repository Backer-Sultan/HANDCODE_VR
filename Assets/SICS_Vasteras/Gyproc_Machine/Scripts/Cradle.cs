...
public class Cradle : MonoBehaviour
{
    ...
    public IEnumerator MoveToRight()
    {
        ...
        audioSource.Play();
        //
        destination = (transform.localPosition.x < middlePosX) ? new Vector3(middlePosX, 0f, 0f) : new Vector3(maxRightPosX, 0f, 0f);
    ...
    public IEnumerator MoveToLeft()
    {
        ...
        audioSource.Play();
        //
        destination = (transform.localPosition.x > middlePosX) ? new Vector3(middlePosX, 0f, 0f) : new Vector3(maxLeftPosX, 0f, 0f);
        while (Vector3.Distance(transform.position, destination) > Vector3.kEpsilon)
        {
            transform.position = Vector3.MoveTowards(transform.localPosition, destination, speed * Time.deltaTime);
            yield return null;
