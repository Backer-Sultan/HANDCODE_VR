...
public class HandCodeVirtualGrasp : MonoBehaviour
{
  ...
  public Transform GetCurrentSelection(uint handID)
  {
    if (handID < 2)
      return current[handID].selectedObject;
    else
      return null;
  }

  public VG_InteractionMode GetCurrentMode(uint handID)
  {
    if (handID < 2)
      return current[handID].mode;
    else
      return VG_InteractionMode.EMPTY;
  }
}
