...
public class ClothAnimation : MonoBehaviour {
  ...
  void Awake()
  {
    ...
    if (cloth)
    {
      ...
      mesh = (Mesh)Instantiate(cloth.GetComponent<SkinnedMeshRenderer>().sharedMesh);

      cloth.GetComponent<SkinnedMeshRenderer>().sharedMesh = mesh;
      coefficients = cloth.coefficients;

      //identification of the cloth animated vertices
      int countAnim = 0, countFixed = 0;
      for (int i = 0; i < coefficients.Length; i++)
      {
        if (coefficients[i].maxDistance == 2)
          countAnim++;

        if (coefficients[i].maxDistance == 0)
          countFixed++;
      }

      int indexAnim = 0, indexFixed = 0;
